...
function renderCurrentShift(rows){
 let open=rows.find(s=>String(s.status||'').toUpperCase()==='ABIERTO' && !s.closed_at);
 if(!open){el('currentShift').className='panel';el('currentShift').innerHTML='<b>SIN TURNO ABIERTO</b><div class="small">No existe un turno ABIERTO sincronizado en Supabase.</div>';return}
 let opened=new Date(open.opened_at),mins=Math.max(0,Math.floor((Date.now()-opened)/60000));
 if(mins>1440){el('currentShift').className='panel alert';el('currentShift').innerHTML='<b>⚠️ REVISAR TURNO — '+(open.operator_name||'')+'</b><div class="small">Supabase lo marca ABIERTO desde hace más de 24 horas. No se presenta como turno actual normal.</div>';return}
 el('currentShift').className='panel shiftopen';
 el('currentShift').innerHTML='<div class="rowtop"><span>🟢 TURNO ABIERTO — '+(open.operator_name||'')+'</span><span>#'+open.local_shift_id+'</span></div><div class="value">'+money(open.revenue_total)+'</div><div class="small">Abierto '+opened.toLocaleString('es-CO')+' · '+Math.floor(mins/60)+' h '+(mins%60)+' min · '+(open.operations||0)+' tickets de parqueo · Base '+money(open.opening_cash)+'</div>';
}
...
function renderManagerHistory(rows){
 let box=document.getElementById('managerHistory'); if(!box)return; box.innerHTML='';
 rows.forEach(r=>{let d=document.createElement('div');d.className='row';d.innerHTML='<div class="rowtop"><span>'+r.movement_type+' · '+(r.plate||'')+'</span><span>'+money(r.amount)+'</span></div><div class="small">'+(r.vehicle_type||'')+' · '+new Date(r.occurred_at).toLocaleString('es-CO')+' · '+(r.operator_out||'')+' · '+(r.payment_method||'')+(r.helmets?' · Cascos '+r.helmets:'')+'</div>';box.appendChild(d)}); window._rbHistory=rows;
}
